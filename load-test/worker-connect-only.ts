...
import { makeSessionId, makeDeviceId, makeSessionInitPayload } from './payloads.js'
...
async function runConnection(config: WorkerConfig): Promise<void> {
  ...
  return new Promise<void>((resolve) => {
    ...
    socket.on('kill', () => {
      emit({ type: 'session_abort', reason: 'killed' })
      clearTimeout(timeout)
      finish()
    })
    ...
    socket.on('connect', async () => {
      ...
      try {
        // Send session_init only — no data batches
        socket.emit('session_init', makeSessionInitPayload(sessionId, deviceId, config.appKey))
        emit({ type: 'init_sent' })

        // Hold connection open for the configured duration
        const pingInterval = 5000
        const pingsToSend = config.sendPings ? Math.floor(config.holdDurationMs / pingInterval) : 0

        for (let i = 0; i < pingsToSend; i++) {
        ...
        const elapsed = pingsToSend * pingInterval
        const remaining = config.holdDurationMs - elapsed
