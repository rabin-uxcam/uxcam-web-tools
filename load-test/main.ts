...
// npx tsx main.ts --app-key [APPKEY]  --connections 10 --ramp-rate 5 --batches 3   
...
const BATCH_INTERVAL = parseInt(args['batch-interval'] || '200', 10)
const SEND_PINGS = args['no-pings'] !== 'true'

if (!APP_KEY) {
  console.error('Usage: npx tsx main.ts --app-key KEY [--url URL] [--connections N] [--workers N] [--ramp-rate N] [--batch-interval MS] [--no-pings]')
  process.exit(1)
}
...
function printProgress() {
  ...
  const line = `  [${elapsed}s] conn=${m.connected}/${TOTAL_CONNECTIONS} err=${m.connectErrors} init=${m.initsSent} data=${m.dataBatches} (${fmtBytes(m.bytesOut)}) disc=${m.disconnected}`
  ...
  return new Promise((resolve, reject) => {
    ...
    const worker = new Worker(
      ...
      `,
      {
        ...
        workerData: {
          ...
          connectionCount,
          rampDelayMs,
          batchIntervalMs: BATCH_INTERVAL,
    ...
    worker.on('message', (msg: { type: string; [key: string]: unknown }) => {
      switch (msg.type) {
        ...
        case 'data_sent': m.dataBatches++; m.bytesOut += msg.bytes as number; break
...
async function main() {
  console.log(`\n  Load test: ${TOTAL_CONNECTIONS} connections → ${WS_URL} (key=${APP_KEY})`)
  console.log(`  ${WORKER_COUNT} workers, ${RAMP_RATE}/s ramp, data from test.json\n`)
  ...
  if (m.errors.length > 0) {
    // Deduplicate errors by message pattern
